class Executable < ActiveRecord::Base
	def self.processEXE ioStream
		fileName = ioStream
	end
	
	#Writes file to disk and then returns an IO stream on that file
	def self.saveToDisk(upload)
		name =  upload.original_filename + " - " + Time.now.to_s
		directory = "public/uploads/executables"
...
def main
	...
	pe = PEParser.new(File.open("exe.txt", "rb"))
